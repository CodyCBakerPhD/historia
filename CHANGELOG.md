...
- Simplified CLI option names to one-word forms with no internal dashes (for example `--projecturl` and `--enddateplaceholderdays`) while preserving snake_case keyword routing in command handlers. ([#46](https://github.com/CodyCBakerPhD/historia/pull/46))
