...
- Unpacked project date updates to `historia.project.update_project_item_dates` and simplified API docs navigation by removing the extra `historia.project.update` layer from top-level docs while keeping the legacy import path available. ([#34](https://github.com/CodyCBakerPhD/historia/pull/34))
