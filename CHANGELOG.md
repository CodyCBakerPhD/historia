...
- `CodyCBakerPhD/historia/action` now checks out and pushes the data repository with the workflow's own `GITHUB_TOKEN` instead of the personal access token, so the personal token never pushes anywhere. The `token` input only needs the classic `project` scope, plus `repo` if any tracked repository is private. GitHub offers no fine-grained permission for Projects, so the token must be classic. This is a breaking change for workflows already using the action: the job must grant `permissions: contents: write`, as the action README's new Setup section and the workflow generated by `historia setup automation` now do. ([#180](https://github.com/CodyCBakerPhD/historia/pull/180))
