...
- `CodyCBakerPhD/historia/action` now checks out and pushes the data repository with the workflow's own `GITHUB_TOKEN` instead of the personal access token, so the personal token never pushes anywhere. For a board owned by an organization, `token` can be a fine-grained token limited to selected repositories, read-only on them, with the organization's `Projects` permission. For a board owned by a user account it must be a classic `project`-scoped token, plus `repo` for private repositories, because GitHub offers no fine-grained permission for user-owned Projects. This is a breaking change for workflows already using the action: the job must grant `permissions: contents: write`, as the action README's new Setup section and the workflow generated by `historia setup automation` now do. ([#180](https://github.com/CodyCBakerPhD/historia/pull/180))
...
- Deprecated `historia setup automation`. It still creates the repository, the project board, the secret, and the workflow file, but the workflow it writes is now short enough to add by hand, so the command prints a deprecation notice and will be removed in a future release. ([#177](https://github.com/CodyCBakerPhD/historia/pull/177))
- Added vendored container actions under `action/` (`update-github`, `project-populate`, and `project-update-dates`) that run **Historia** from the pinned GHCR image, and switched the workflow generated by `historia setup automation` to call them. The generated workflow no longer sets up Python, installs the package, or caches anything to do with pip, which removes the install fragility behind the exit code 127 failures entirely. Each action tag pins the image built for that same release, so `@v0.10.14` runs `ghcr.io/codycbakerphd/historia:0.10.14`. ([#177](https://github.com/CodyCBakerPhD/historia/pull/177))
- Removed the `python_version` parameter from `provision_automation` and its wizard prompt, along with the `Requires-Python` and Python classifier lookups that only served it. The generated workflow no longer contains a Python installation to configure, so the setting had nothing left to affect. This is a breaking change for anything calling `provision_automation` with `python_version`. ([#177](https://github.com/CodyCBakerPhD/historia/pull/177))

### 🐛 Bug Fix

- Fixed the workflow generated by `historia setup automation` failing with `historia: cannot execute: required file not found` (exit code 127) once the runner image bumps its Python patch release. The pip cache no longer persists the installed `~/.local` tree, whose console script shebangs point at an interpreter path that no longer exists after such a bump. The `Install historia` step also no longer skips itself on a cache hit, and the cache key no longer relies on `hashFiles` against a workflow file that is never checked out into the workspace. That key silently resolved to an empty hash, making every restore an exact hit, so the install step never ran again and a bumped `HISTORIA_SPEC` had no effect. ([#176](https://github.com/CodyCBakerPhD/historia/pull/176))
...
- Updated `historia project populate` to build the `Adding items to project` iteration from only URLs that are not already in the target project, while preserving member-field merge updates for existing items. ([#147](https://github.com/CodyCBakerPhD/historia/pull/147))
- Reduced `historia project populate --members` runtime by skipping no-op member-field updates for items whose existing `Members` value already matches inferred usernames. ([#151](https://github.com/CodyCBakerPhD/historia/pull/151))
...
- Added explicit empty-token permissions to the daily link checker notification job so all email-only notify jobs follow the same least-privilege pattern. ([#145](https://github.com/CodyCBakerPhD/historia/pull/145))
