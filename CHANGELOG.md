...
- Split the single `token` of `CodyCBakerPhD/historia/action` into three so that each step holds only the access its job needs: `token` fetches the activity with a read-only fine-grained token, the new `project-token` updates the project board with a classic `project`-scoped token, and the new `repository-token` checks out and pushes the data repository with the run's own `GITHUB_TOKEN`. No personal token can push to any repository. Both new inputs fall back to `token` when empty, so a workflow passing one classic `repo` and `project` token keeps working unchanged. The action README's Tokens section is now the single place documenting how to create them. ([#180](https://github.com/CodyCBakerPhD/historia/pull/180))
