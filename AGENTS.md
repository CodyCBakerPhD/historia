...
-   For internal imports, always use the relative import style (e.g., `from .foo import bar`); when monkeypatching such imports in tests, target the importing module's binding (e.g., `historia.data.github._dump.fetch_info_for_date`), not the original definition module
