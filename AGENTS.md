...
- Release by publishing the draft that `Prepare release draft` keeps on every merge to `main`. Its tag name and target come from `pyproject.toml`, so the tag is never typed. Rename the `## Upcoming` heading in `CHANGELOG.md` to that version before publishing. Do not create the tag by hand: the version is bumped once per PR, so a name chosen after the next merge is a version that was never built, which is how `v0.10.15` was cut on a tree bumped to `0.10.16`.
