...
- Bump the version in `pyproject.toml` once per PR when any file under `src/` or `pyproject.toml` itself changes. That is the only place to edit it. The vendored actions are versioned separately and must never be bumped along with it. They pin one published image, reach each other by `@v0`, and the documentation references `@v0`, all of which stay put while the package releases. Changing the actions means cutting the next major tag and choosing the image that tag needs, so the image pin is only ever edited as part of that. Do NOT bump for changes that are purely CI/workflow, documentation, or configuration (e.g., GitHub Actions workflows, `AGENTS.md`, `README.md` badges).
