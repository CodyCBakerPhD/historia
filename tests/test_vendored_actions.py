...
def _released_versions() -> set[str]:
    """Release tags are absent from a shallow checkout, so callers treat an empty set as unknown."""
    ...
    return {tag.removeprefix("v") for tag in completed_process.stdout.split()}
...
@pytest.mark.ai_generated
def test_vendored_actions_pin_one_released_image() -> None:
    """
    The actions name the image they were built against, and it must already be published.

    That pin is deliberate rather than derived. It changes when the actions are given a new major tag,
    not when the package releases, so a workflow keeps running what its tag was built against. Naming
    an unpublished version is the one way to get it wrong, since the tag would resolve for nobody.
    """
    ...
    released_versions = _released_versions()
    if released_versions:
        assert pinned_versions <= released_versions, sorted(pinned_versions - released_versions)
